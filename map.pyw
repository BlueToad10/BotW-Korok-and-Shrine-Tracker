...
WINDOWSIZE2 = (WINDOWSIZEX+65, WINDOWSIZEY)
Version = "Version 1.2"
...
ColorMode_List = pygame.sprite.Group()#ColorMode
...
ColorModeImage = pygame.transform.scale(pygame.image.load("assets/background numbers.png"), (WINDOWSIZEX, WINDOWSIZEY)).convert_alpha()
...
class button(pygame.sprite.Sprite):
    ...
    def update(self):
        global korokSeeds, shrines, colormode
        if self.value > -1:
            DrawTextFileFont(str(shrineCounters[self.value]) + "/ " + str(shrineCountersMax[self.value]), "font.ttf", 26, mapColours[self.value], windowSurface, self.x+30, self.y, self.x+90+35, self.y+30)
            DrawTextFileFont(str(seedCounters[self.value]) + "/" + str(seedCountersMax[self.value]), "font.ttf", 26, mapColours[self.value], windowSurface, self.x+30, self.y+30, self.x+90+35, self.y+30)
            if colormode == False:
                DrawTextFileFont(str(self.value+1), "font.ttf", 26, (255,255,255), windowSurface, self.x+105, self.y, self.x+90+60, self.y+30)
...
colormode=buttonsmode=True
def controls():
    global mouseposx, mouseposy, colormode, windowSurface, buttonsmode
    LEFT = 1
    RIGHT = 3
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == ord("m"):
                if buttonsmode == True:
                    buttonsmode = False
                elif buttonsmode == False:
                    buttonsmode = True
            if event.key == K_SPACE:
                if colormode == True:
                    colormode = False
                    windowSurface = pygame.display.set_mode(WINDOWSIZE2)
...
ColorModeBackground = image(ColorModeImage, 0, 0)
Background_List.add(Background, Background2)
ColorMode_List.add(ColorModeBackground)
...
while True:
    ...
    if colormode == False:
        ColorMode_List.draw(windowSurface)
